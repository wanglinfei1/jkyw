...
//医生信息列表展示
//获取图文咨询列表
function getList(pageNum,size,isSupportTel,isSupportVideoTex,isSupportBespoke){
    $.ajax({
        type:"post",
        ...
        data:{
            "pageNum":pageNum,
            "pageSize":size,
            "isSupportTel":isSupportTel,
            "isSupportVideoTex":isSupportVideoTex,
            "isSupportBespoke":isSupportBespoke
        },
        beforeSend:function(){
            $('<img src="../../img/loading.gif" class="loading"/>').appendTo('.docList');
        },
        success:function(data){
            ...
            /*//style();*/
            pages(data.data.pages,size,isSupportTel,isSupportVideoTex,isSupportBespoke);
            function docDetails() {
...
//分页
var iNum=0;
function pages(iN,size,isSupportTel,isSupportVideoTex,isSupportBespoke){
    $('.page1 div').html('');
    ...
    $('.page1 div a').on('click',function(){
        ...
        iNum=$(this).index();
        $('.page1 div a').eq(iNum).addClass('active');
        getList(iNum+1,size,isSupportTel,isSupportVideoTex,isSupportBespoke);
    });
